...
export function useProjectsService() {
    ...
    async function getProjectsByFilters(status: number): Promise<ApiResponse<IProject[]>> {
        const endpoint: string = `/get-by-filters?status=${status}`;
        return post(`${projectsUrl}${endpoint}`);
    }

    return { GetProjectByUser, getProjectsByFilters, GetProjectById, GetProjectFiles, CreateProject, UpdateProject, DeleteProject, GetAllStatus, DeleteFileProject, FinishProject }
}